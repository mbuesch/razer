                                       ...
                                       struct deathadder2013_command *cmd)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        err = deathadder2013_usb_write(priv, LIBUSB_REQUEST_SET_CONFIGURATION,
                                       0x300, cmd, sizeof(*cmd));
        if (err)
            return err;
        err = deathadder2013_usb_read(priv, LIBUSB_REQUEST_CLEAR_FEATURE,
                                      0x300, cmd, sizeof(*cmd));
        if (err)
            return err;
        if (cmd->status != 3 &&
            ...
            razer_error("razer-deathadder2013: Command %04X/%04X failed with %02X\n",
                        le16_to_cpu(cmd->command), le16_to_cpu(cmd->request), cmd->status);
        }
...
static enum razer_mouse_freq deathadder2013_get_freq(struct razer_mouse_profile *p)
{
...
static struct razer_mouse_profile * deathadder2013_get_profiles(struct razer_mouse *m)
{
    struct deathadder2013_private *priv = m->drv_data;
        ...
        struct libusb_device *usbdev)
{
    ...
    razer_generic_usb_gen_idstr(usbdev, m->usb_ctx->h,
                                "DeathAdder 2013 Edition", 1, NULL, m->idstr);
