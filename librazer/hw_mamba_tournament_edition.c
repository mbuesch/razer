...
enum mamba_te_led_state_option // for further implementation
{
...
{
	MAMBA_TE_REQUEST_INIT = 0x0004, // Original 0x0004
...
static int mamba_te_usb_action
				 ...
				 struct mamba_te_command *cmd)
{
	...
	err = libusb_control_transfer (m->usb_ctx->h, direction
									| LIBUSB_REQUEST_TYPE_CLASS
									| LIBUSB_RECIPIENT_INTERFACE, request, command, 0, (unsigned char *)cmd, sizeof(*cmd),
	RAZER_USB_TIMEOUT);
	razer_event_spacing_leave (&drv_data->packet_spacing);

	if (err != sizeof(*cmd))
	{
		razer_error("razer-mamba-tournament-edition: " "USB %s 0x%01X 0x%02X failed with %d\n",
				direction == LIBUSB_ENDPOINT_IN ? "read" : "write", request, command, err);
		return err;
	}
...
static int mamba_te_send_command (struct razer_mouse *m, struct mamba_te_command *cmd)
{
	int err; //
	uint8_t checksum;
	...
	{
		razer_error("razer-mamba-tournament-edition: " "Command %02X %04X bad response checksum %02X " "(expected %02X)\n",
					cmd->size, be16_to_cpu (cmd->request), checksum, cmd->checksum);

		return -EBADMSG;
	}

	if (cmd->status != MAMBA_TE_SUCCESS_STATUS)
		razer_error("razer-mamba-tournament-edition: " "Command %02X %04X failed with %02X\n",
...
static int mamba_te_send_get_serial_no_command (struct razer_mouse *m)
{
	...
	strncpy (drv_data->serial, (const char *)cmd.bvalue, MAMBA_TE_REQUEST_SIZE_GET_SERIAL_NO);
	return 0;
}
...
static int mamba_te_send_set_led_state_command (struct razer_mouse *m, struct mamba_te_led *led)
{
	struct mamba_te_command cmd;
	...
	cmd.bvalue[0] = ((cmd.bvalue[0]) & (led->state));
	cmd.bvalue[1] = ((cmd.bvalue[1]) & (led->state));
	return mamba_te_send_command (m, &cmd);
}

static int mamba_te_send_set_led_mode_command (struct razer_mouse *m, struct mamba_te_led *led)
{
	struct mamba_te_command cmd;
...
static int mamba_te_send_set_led_color_command (struct razer_mouse *m, struct mamba_te_led *led)
{
	struct mamba_te_command cmd;
...
static int mamba_te_supported_axes (struct razer_mouse *m, struct razer_axis **res_ptr)
{
	struct mamba_te_driver_data *drv_data;
...
static int mamba_te_supported_dpimappings (struct razer_mouse *m, struct razer_mouse_dpimapping **res_ptr)
{
	struct mamba_te_driver_data *drv_data;
...
static int mamba_te_supported_resolutions (struct razer_mouse *m, enum razer_mouse_res **res_ptr)
{
	size_t i;
	size_t step_number;
...
static int mamba_te_supported_freqs (struct razer_mouse *m, enum razer_mouse_freq **res_ptr)
{
...
static struct razer_mouse_dpimapping *mamba_te_get_dpimapping (struct razer_mouse_profile *p, struct razer_axis *axis)
{
	struct mamba_te_driver_data *drv_data;
...
static int mamba_te_led_toggle_state (struct razer_led *led, enum razer_led_state new_state)
{
	struct mamba_te_driver_data *drv_data;
...
static int mamba_te_led_change_color (struct razer_led *led, const struct razer_rgb_color *new_color)
{
	struct mamba_te_driver_data *drv_data;
	struct mamba_te_led *priv_led;
...
static int mamba_te_set_freq (struct razer_mouse_profile *p, enum razer_mouse_freq freq)
{
	struct mamba_te_driver_data *drv_data;
	...
	if (freq != RAZER_MOUSE_FREQ_125HZ && freq != RAZER_MOUSE_FREQ_500HZ && freq != RAZER_MOUSE_FREQ_1000HZ)
		return -EINVAL;
...
static int mamba_te_led_set_mode (struct razer_led *led, enum razer_led_mode new_mode)
{
	int err;
	struct mamba_te_driver_data *drv_data;
	struct mamba_te_led *priv_led;
...
static int mamba_te_get_leds (struct razer_mouse *m, struct razer_led **leds_list)
{
	...
	led_state = drv_data->led.state == MAMBA_TE_LED_STATE_OFF ? RAZER_LED_OFF : RAZER_LED_ON;
...
int razer_mamba_te_init (struct razer_mouse *m, struct libusb_device *usbdev)
{
	int err;
	size_t i;
	struct mamba_te_driver_data *drv_data;
	...
	razer_event_spacing_init (&drv_data->packet_spacing, MAMBA_TE_PACKET_SPACING_MS);
	...
	razer_init_axes (drv_data->axes, "X/Y", RAZER_AXIS_INDEPENDENT_DPIMAPPING, "Scroll", 0, NULL, 0);

	m->drv_data = drv_data;

	if ((err = razer_usb_add_used_interface (m->usb_ctx, 0, 0)) || (err = m->claim (m)))
	{
		free (drv_data);
		return err;
	}
	...
	MAMBA_TE_DEVICE_NAME, false, drv_data->serial, m->idstr);
